...
import { Modal, NativeModules, Pressable, View } from 'react-native';
...
function loadNativeWebRTC(): NativeWebRTC | null {
  if (!NativeModules.WebRTCModule) {
    return null;
  }

  try {
    // Deliberately lazy: Expo Go does not include this native module.
    // A development build does, and then this behaves like an in-app WhatsApp call.
...
export function ChatCallModal({
  ...
  const nativeWebRTCRef = useRef<NativeWebRTC | null>(null);
  ...
  const addIceCandidateSafely = useCallback(async (candidate: RTCIceCandidateInit) => {
    const nativeWebRTC = nativeWebRTCRef.current;
    const peerConnection = peerConnectionRef.current;
    if (!nativeWebRTC || !peerConnection) return;
    if (!peerConnection.remoteDescription) {
      pendingCandidatesRef.current.push(candidate);
      return;
    }
    await peerConnection.addIceCandidate(new nativeWebRTC.RTCIceCandidate(candidate)).catch(() => undefined);
  }, []);
  ...
  const setupPeerConnection = useCallback(async () => {
    const nativeWebRTC = nativeWebRTCRef.current ?? loadNativeWebRTC();
    nativeWebRTCRef.current = nativeWebRTC;
    if (!nativeWebRTC) {
      setErrorHint(labels.nativeBuildRequired);
      setStatus('error');
      return null;
    }

    const peerConnection: any = new nativeWebRTC.RTCPeerConnection({
      iceServers: [{ urls: 'stun:stun.l.google.com:19302' }],
    });
    peerConnectionRef.current = peerConnection;

    peerConnection.onicecandidate = (event: { candidate?: { toJSON: () => RTCIceCandidateInit } | null }) => {
      if (event.candidate) {
    ...
    (peerConnection as any).ontrack = () => {
      markCallAnswered();
    };

    try {
      const localStream = await nativeWebRTC.mediaDevices.getUserMedia({ audio: true, video: false });
  ...
  }, [cleanup, labels.connectTimeout, labels.microphoneError, labels.nativeBuildRequired, markCallAnswered, sendSignal]);
  ...
  const acceptIncomingCall = useCallback(async () => {
    ...
    await peerConnection.setRemoteDescription(new nativeWebRTC.RTCSessionDescription({ type: 'offer', sdp: incomingOffer.sdp }));
    ...
    return realtimeService.onCallSignal((signal) => {
      ...
      if (signal.type === 'answer' && signal.sdp) {
        ...
        void peerConnectionRef.current?.setRemoteDescription(
          new nativeWebRTC.RTCSessionDescription({ type: 'answer', sdp: signal.sdp }),
        ).then(() => {
          markCallAnswered();
